...
### 2026-08-26（二）— 粘贴区「MD 源码模式」开关 + 上传区紧凑化

- **修改内容**：
  1. **MD 源码模式开关**：输入区标题栏新增 Apple 风格拨动开关；开启后粘贴强制走纯文本路径（跳过富文本 HTML 接收），避免剪贴板富文本把 `#`、`>`、`|` 等 MD 语法标记丢掉；粘贴时自动剥离 YAML front matter
  2. **开关切换即时生效**：开启开关时对已输入的纯文本补剥 front matter 并重渲染，无需重新粘贴
  3. **上传区紧凑化**：由大块卡片改为单行紧凑条（两个小按钮 + 行内「或拖拽文件到此处」提示），让粘贴输入区成为视觉主体
  4. **占位文案更新**：placeholder 提示粘贴 MD 源码需开启「MD 源码模式」
- **修改原因**：用户要求粘贴区也支持可选 MD 格式转换入口（与文件上传入口同理：显式声明格式，降低误判）；上传区应缩小，因为粘贴文字的输入框才是主要区域
- **影响范围**：`paste` 事件处理、`mdPasteMode` 开关监听、`.upload-area`/`.upload-btn`/`.md-mode-toggle` CSS、panel-title 布局、placeholder 文案
